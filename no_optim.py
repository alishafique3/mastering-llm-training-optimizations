...
def create_dataloader(txt, tokenizer, batch_size=4, max_length=1024,
    ...
    return DataLoader(
        ...
        drop_last=drop_last, num_workers=0,   # 🐢 single-process data loading
...
def calc_loss_batch(input_batch, target_batch, model, device):
    input_batch = input_batch.to(device)       # 🐢 blocking transfer
...
def train_model(model, train_loader, val_loader, optimizer, device,
    ...
    # ⏱️ same timing as optimized version (CUDA events on GPU, wall clock on CPU)
...
def main(settings):
    ...
    # 🐢 no torch.compile, no gradient checkpointing tweaks, no fused optimizer
